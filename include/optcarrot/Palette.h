...
public:
  ...
  static constexpr std::array<std::tuple<uint8_t, uint8_t, uint8_t>, 512> defactoPalette() {
    ...
    for (const auto &f: map) {
      ...
      for (const auto &rgb: palette) {
        uint8_t r = std::min(static_cast<uint8_t>((rgb >> 16 & 0xff) * rf), static_cast<uint8_t>(0xff));
        uint8_t g = std::min(static_cast<uint8_t>((rgb >>  8 & 0xff) * gf), static_cast<uint8_t>(0xff));
        uint8_t b = std::min(static_cast<uint8_t>((rgb >>  0 & 0xff) * bf), static_cast<uint8_t>(0xff));
        ret.at(i++) = {r,g,b};
      }
    }
    return ret;
  ...
  static constexpr std::array<std::tuple<uint8_t, uint8_t, uint8_t>, 512> nestopiaPalette() {
    std::array<std::tuple<uint8_t, uint8_t, uint8_t>, 512> ret{};
    return ret;
    /*
    (0..511).map do |n|
